...
/* eventsource.min.js */
;(function(global){if("EventSource"in global)return;var reTrim=/^(\s|\u00A0)+|(\s|\u00A0)+$/g;var EventSource=function(url){var eventsource=this,interval=500,lastEventId=null,cache='';if(!url||typeof url!='string'){throw new SyntaxError('Not enough arguments');}
...
data.settings=settings;overlay.data("loadingoverlay_data",data);container.data("loadingoverlay",overlay);overlay.fadeTo(0,0.01).appendTo("body");_IntervalResize(container,true);if(settings.resizeInterval>0){data.resizeIntervalId=setInterval(function(){_IntervalResize(container,false);},settings.resizeInterval);}
...
/* eload.js */
function subvpayment(){var vpayment=$("#vpayment").val();var vip=$("#vip").val();var vmac=$("#vmac").val();$("#subvpayment").attr("disabled",true);$.ajax({type:"POST",url:"/admin/index?pop=1&lpb=subvpayment",data:{vpayment:vpayment,vmac:vmac,vip:vip},success:function(msg){if(msg=="1"){document.getElementById("vpayment").value="";toastr.success("Voucher code "+vpayment+" successfully redeemed","",{"timeOut":"1000","positionClass":"toast-top-right"});$("#subvpayment").attr("disabled",false);}
else{document.getElementById("vpayment").value="";toastr.error(msg,'Error');$("#subvpayment").attr("disabled",false);}}});}
function trignav(){$('.nav-link').click(function(){var target=$(this).data('target');$('.collapse').not(target).slideUp();$(target).slideToggle();});}
function gcashproduct(){var mac=$('#mac').val();var ip=$('#ip').val();$.LoadingOverlay("show");var gcashnumber=$('#gcashnumber').val();var gcashexpressid=$('#gcashexpressid').val();var gcashfirstname=$('#gcashfirstname').val();var gcashlastname=$('#gcashlastname').val();if(gcashnumber!=''){$.post('admin/index?pop=1&lpb=gcashproduct',{gcashnumber:gcashnumber,gcashexpressid:gcashexpressid,gcashfirstname:gcashfirstname,gcashlastname:gcashlastname,ip:ip,mac:mac},function(data){$('#gcashannouncer').html(data);$('#gcashproduct').modal({backdrop:'static',keyboard:false});$('#gcashcashin').slideDown();$.LoadingOverlay("hide");});}else{location.reload();}}
function eloadproduct(){var mac=$('#mac').val();var ip=$('#ip').val();$.LoadingOverlay("show");var eloadnumber=$('#eloadnumber').val();if(eloadnumber!=''){$.post('admin/index?pop=1&lpb=eloadproduct',{eloadnumber:eloadnumber,ip:ip,mac:mac},function(data){$('#eloadannouncer').html(data);$('#eloadproduct').modal({backdrop:'static',keyboard:false});$('#regular').slideDown();trignav();$.LoadingOverlay("hide");});}else{location.reload();}}
var sound=new Howl({src:['assets/images/email-notif.mp3'],});var eloadinsertcoin=new Howl({src:['assets/images/insertcoin.mp3'],});var eloadnarrate=new Howl({src:['assets/images/narrate.mp3'],});function balanceinquiry(){var mac=$('#mac').val();var ip=$('#ip').val();var ieloadnumber=$('#ieloadnumber').val();$.LoadingOverlay("show");if(ieloadnumber!=''){$.post('admin/index?pop=1&lpb=balanceinquiry',{ieloadnumber:ieloadnumber,ip:ip,mac:mac},function(data){$('#balanceinquiry_announcer').html(data);$('#balancecheck').modal({backdrop:'static',keyboard:false});$.LoadingOverlay("hide");});}else{location.reload();}}
function procgcash(ip,mac,mnumber,pcode,network,coinslot,gcashexpressid,gcashfirstname,gcashlastname){$.LoadingOverlay("show");$.post('admin/index?pop=1&lpb=autoproc',{ip:ip,mac:mac,mnumber:mnumber,pcode:pcode,network:network,coinslot:coinslot,gcashexpressid:gcashexpressid,gcashfirstname:gcashfirstname,gcashlastname:gcashlastname},function(data){var success=data.trim();if(success=='1'){$('#eloadprocess').modal('hide');$('#eloadcomplete').modal('show');eloadnarrate.play();$.LoadingOverlay("hide");}else{$('#eloaderror').html(data);$.LoadingOverlay("hide");}});}
function proceload(ip,mac,mnumber,pcode,network,coinslot){$.LoadingOverlay("show");$.post('admin/index?pop=1&lpb=autoproc',{ip:ip,mac:mac,mnumber:mnumber,pcode:pcode,network:network,coinslot:coinslot},function(data){var success=data.trim();if(success=='1'){$('#eloadprocess').modal('hide');$('#eloadcomplete').modal('show');eloadnarrate.play();$.LoadingOverlay("hide");}else{$('#eloaderror').html(data);$.LoadingOverlay("hide");}});}
function selectcoinslotgcash(ip,mac,pcode,mnumber,network,category,price,pdescription,gcashexpressid,gcashfirstname,gcashlastname){var mac=$('#mac').val();$.LoadingOverlay("show");if(mac!=''){$.post('admin/index?pop=1&lpb=selectcoinslotgcash',{ip:ip,mac:mac,pcode:pcode,mnumber:mnumber,network:network,category:category,price:price,pdescription:pdescription,gcashexpressid:gcashexpressid,gcashfirstname:gcashfirstname,gcashlastname:gcashlastname},function(data){$('#multicoinslot_announce').html(data);$('#eloadgcashmulticoinslot').modal({backdrop:'static',keyboard:false});});}}
function selectcoinslot(ip,mac,pcode,mnumber,network,category,price,pdescription){var radiuscode=$('#radiuserverid').val();$.LoadingOverlay("show");if(radiuscode!=''){$.post('admin/index?pop=1&lpb=selectcoinslot',{ip:ip,mac:mac,pcode:pcode,mnumber:mnumber,network:network,category:category,price:price,pdescription:pdescription},function(data){$('#multicoinslot_announce').html(data);$('#eloadmulticoinslot').modal({backdrop:'static',keyboard:false});});}}
function eloadprocess(ip,mac,pcode,mnumber,network,category,coinslot,gcashexpressid,gcashfirstname,gcashlastname){var radiuscode=$('#radiuserverid').val();$.LoadingOverlay("show");if(radiuscode!=''){$.post('admin/index?pop=1&lpb=eloadprocess',{ip:ip,mac:mac,pcode:pcode,mnumber:mnumber,network:network,category:category,coinslot:coinslot,gcashexpressid:gcashexpressid,gcashfirstname:gcashfirstname,gcashlastname:gcashlastname,},function(data){$('#eloadprocess_announce').html(data);$('#eloadprocess').modal({backdrop:'static',keyboard:false});var tamount="";if(typeof(EventSource)!=="undefined"){var IrequestSource=new EventSource("admin/index?insertcoin=1&ip="+ip+"&mac="+mac+"");IrequestSource.addEventListener('tamount',function(event){if(tamount!=event.data){tamount=event.data;document.getElementById("eloadamount").innerHTML=tamount;sound.play();isend(coinslot);}},false);}
if(!$('#eloadproduct').hasClass('show')){eloadinsertcoin.play();$.LoadingOverlay("hide");}});}}
function isend(coinslot){$.ajax({type:"GET",url:"admin/index?isend=1&func=isend&coinslot="+coinslot+""});}
function getnumbers(myfield,e,dec){var key;var keychar;if(window.event)
key=window.event.keyCode;else if(e)
key=e.which;else
return true;keychar=String.fromCharCode(key);if((key==null)||(key==0)||(key==8)||(key==9)||(key==27)||keychar==".")
return true;if(key==13){if(popvisible==0){get_products();popvisible=1;return true;}}else if((("0123456789").indexOf(keychar)>-1))
return true;else
return false;}
/* lpb.js */
window.navigator.standalone;if(typeof(EventSource)=="undefined"){window.location.href='/lite';}
function applyvoucher(vcode,time){if(confirm('Apply voucher: '+vcode+' ('+time+')?')){$('#vcode').val(vcode);$('#sub-voucher').trigger('click');}else{return false;}}
$("#pfunc").click(function(e){$.ajax({type:"POST",url:"/admin/index?pfunc=1",success:function(msg){if(msg=="1"){toastr.success("You Sucessfully Pause your time!","",{"timeOut":"1000","positionClass":"toast-top-right"});location.reload();}else if(msg=="2"){toastr.success("You Sucessfully Resume your time!","",{"timeOut":"1000","positionClass":"toast-top-right"});location.reload();}else if(msg=="3"){toastr.error('Error!','error');}
else{toastr.error(msg,'Sorry!');}}});});var timer=parseInt($('div[usertime]').attr("usertime"));$("#btn-vcode").click(function(e){var amountminutes='';var minutes='';amountminutes=$("#amountminutes").val();minutes=$("#minutes").val();$.ajax({type:"POST",url:"/admin/index?sconvert=1",data:{amountminutes:amountminutes},success:function(msg){if(msg=="1"){toastr.success("Sucessfully Convert Time To Voucher!","",{"timeOut":"1000","positionClass":"toast-top-right"});var r=document.getElementById("minutes");r.value=minutes-amountminutes;timer=timer-amountminutes*60;}
else{toastr.error(msg,'Error');}}});});$('.collapse').on('show.bs.collapse',function(){$('.collapse.show').each(function(){$(this).collapse('hide');});});$('div[usertime]').each(function(){var countdown=setInterval(function(){timer=timer-1;var days=Math.floor(timer/(3600*24));var hours=Math.floor(timer/3600)%24;var minutes=Math.floor(timer/60)%60;var seconds=Math.floor(timer)%60;if(timer>=86400){var result="<b> "+days+"</b> d. <b>"+hours+"</b> hr.<b>"+minutes+"</b>min. <b>"+seconds+"</b> sec";}else if(timer>=3600&&timer<=86400){var result="<b>"+hours+"</b> hr. <b>"+minutes+"</b> min. <b>"+seconds+"</b> sec";}else if(timer>=1&&timer<=3600){var result="<b>"+minutes+"</b> min. <b>"+seconds+"</b> sec";}else{var result='<h2 class="paused ptime"> -- </h2>';}
$('#utimer').html(result);},1000);});$(".btn-spinner").on('click',function(e){$(".btn-spinner").attr("disabled","disabled");$(this).prepend('<i class="fa fa-spinner fa-spin"></i> ');});$("#close-eload").on('click',function(e){$(".eload-spinner").attr("disabled",false);});$('#buyload').on('hide.bs.modal',function(e){$(".eload-spinner").attr("disabled",false);$(".fa-spin").remove();});$('#eloadcomplete').on('hide.bs.modal',function(e){location.reload();});$(".eload-spinner").on('click',function(e){e.preventDefault();$(".eload-spinner").attr("disabled","disabled");$(this).prepend('<i class="fa fa-spinner fa-spin"></i> ');});$("#sub-voucher").click(function(e){var vcode='';vcode=$("#vcode").val();$("#sub-voucher").attr("disabled",true);$.ajax({type:"POST",url:"/admin/index?svouchers=1",data:{vcode:vcode},success:function(msg){if(msg=="1"){$("#rtime").html('<div class="spinner-border text-primary" role="status"> <span class="sr-only">Loading...</span> </div>');document.getElementById("vcode").value="";toastr.success("Voucher code "+vcode+" successfully redeemed","",{"timeOut":"1000","positionClass":"toast-top-right"});$("#sub-voucher").attr("disabled",false);}
else{document.getElementById("vcode").value="";toastr.error(msg,'Error');$("#sub-voucher").attr("disabled",false);}}});});$("#logoutnow").click(function(e){$.ajax({type:"POST",url:"/admin/index?sconvert=1",data:{logout:"1"},success:function(msg){if(msg=="1"){$("#rtime").html('<div class="spinner-border text-primary" role="status"> <span class="sr-only">Loading...</span> </div>');toastr.success("Time Logout!","",{"timeOut":"1000","positionClass":"toast-top-right"});location.reload();}}});});function isend(coinslot){$.ajax({type:"GET",url:"/admin/index?isend=1&func=isend&coinslot="+coinslot+""});}
function timeconvertion($minutes){var d=Math.floor($minutes/1440);var h=Math.floor(($minutes-d*1440)/60);var m=$minutes-(d*1440)-(h*60);if($minutes>=1440){return""+d+"d:"+h+"h:"+m+"m";}else if($minutes>=60&&$minutes<=1440){return""+h+"h:"+m+"m";}else{return""+m+"m";}}
/* standalone.js */
